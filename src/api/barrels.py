...
def get_wholesale_purchase_plan(wholesale_catalog: list[Barrel]):
    ...
    try:
        with db.engine.connect() as connection:
            ...
            #     "SELECT red, green, blue, dark FROM current_ingredient_levels"
            # )).mappings()
            # # inventory = result.fetchone()
            # if not ingredient_levels:
            #     raise HTTPException(status_code=500, detail="Inventory not found.")
            # print(Fore.YELLOW + f"Fetching inventory: {ingredient_levels}" + Style.RESET_ALL)
            # num_green_ml = ingredient_levels.fetchone()["green"]
            # num_blue_ml = ingredient_levels.fetchone()["blue"]
            # num_red_ml = ingredient_levels.fetchone()["red"]
            # num_dark_ml = ingredient_levels.fetchone()["dark"]
